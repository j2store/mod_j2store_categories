 ...
 * @since       1.5
 */
class ModJ2storeCategoriesHelper
{
    /**
    ...
    public static function getList(&$params)
    {
        ...
        foreach($items as $key=>$item){
            $item->getParams = ModJ2storeCategoriesHelper::getParams($item->params);
            foreach($menu_items as $m_item){
                if(isset($m_item->query['catid'])){
                    if(!is_array($m_item->query['catid'])){
                        $m_item->query['catid'] = array($m_item->query['catid']);
                    }
        ...
        if ($category_display_view != "grid_view") {
            ...
            $items = ModJ2storeCategoriesHelper::getChildren($items, $id);
        }
     ...
     * @return array
     */
    public static function getChildren($elements, $parentId = 0)
    {
        $branch = array();
        foreach ($elements as $element) {
            if ($element->parent_id == $parentId) {
                $children = ModJ2storeCategoriesHelper::getChildren($elements, $element->id);
                if ($children) {
    ...
    /**
     * Params decode using joomla registry
     *
     * @param $params
     * @return object
     */
